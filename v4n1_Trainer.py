...
from vulnscan import log, generate_dataset, create_sampler, train_model, plot_training, SimpleNN, \
    EmbeddingDataset, generate_embeddings, cfg
...
def main():
    ...
    # Generate dataset
    texts, labels = generate_dataset(gpt_tokenizer=gpt_tokenizer, gpt_model=gpt_model)
    ...
    generate_embeddings(embed_model=embed_model, texts=train_texts, labels=train_labels)
    log("Generating validation embeddings...")
    generate_embeddings(embed_model=embed_model, texts=val_texts, labels=val_labels)
    ...
    sampler = create_sampler(dataset=train_dataset, model=model_dummy)
    ...
    history = train_model(model=model, train_loader=train_loader, val_loader=val_loader)
...
if __name__ == "__main__":
    cfg.update({
        ...
        # Device / system
        "RAM_THRESHOLD": 0.85  # Maximum allowed fraction of RAM usage before halting generation
    })
    main()
